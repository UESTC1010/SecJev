...
def main():
    ...
    ap.add_argument('--split', choices=['development', 'calibration', 'test'], required=True)
    ap.add_argument('--tag', required=True)
    ap.add_argument('--shard', type=int, required=True)
    ap.add_argument('--shards', type=int, default=3)
    a = ap.parse_args()
    if a.split == 'test':
        assert (R / 'results/frozen-final-candidate.json').exists()
    ...
    ck = Checkpoint(a.checkpoint)
    ck.meta.base = str(BASE)
    ck.meta.base_revision = None
    tok, model = ck.load('cuda:0', LoadOptions(dtype=torch.float32, merge=False, attn='sdpa', temperature=1.0))
    ...
    with torch.inference_mode():
        while cursor < len(ordered):
            ...
            while n > 1 and (n * ordered[cursor + n - 1]['length'] > 4096 or n * ordered[cursor + n - 1]['length'] ** 2 > 4000000):
                n -= 1
            batch = ordered[cursor:cursor + n]
            logits = model.forward_batch([r['enc'] for r in batch])
            torch.cuda.synchronize()
            for row, z in zip(batch, logits):
                values = z[0].float().cpu().tolist()
            ...
            cursor += n
            if cursor % 500 < n:
                print(a.tag, a.split, 'shard', a.shard, cursor, len(ordered), flush=True)
    path = R / 'results' / f'{a.tag}-{a.split}-shard-{a.shard}.jsonl'
